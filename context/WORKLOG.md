...
> - **[opencode]** 改善ループの実施（Human選択「1〜4全て」）。①**検証の穴を塞ぐ回帰テスト**: `e2e/specs/to-top-button-position.spec.js`を新設し、「一番上に戻る」ボタンがスクロール後も全スキン(mono/link-amber/ordinary/terminal)で`position:fixed`/右下(右マージン8〜24)/z-40に留まることを検証。従来は`scroll-keys`/`ui-audit`が標準スキン・未スクロールのみで、今回のバグ（スタイル系スキン限定・スクロール後限定）を検出できなかった。4件成功。②**E2E使い勝手**: (a)`run-e2e.mjs`が単一`--spec`指定時に`E2E_INCLUDE_TEMP=1`を立て、`wdio.conf.js`の`exclude`(zz-temp)を解除→`--exclude "no-such-match"`ハック無しで一時スペックを実行可能に（smokeで確認）。(b)種まき共通化: `seed-reader-data.mjs`に`seedScrollableFeed`/`unseedScrollableFeed`を追加。(c)窓サイズ不定(ウィンドウ状態プラグイン由来)を指摘し、スクロール依存チェックは`scrollTop`直接設定+十分な件数種まきで窓サイズ非依存にする指針をVERIFYへ明記。③**規約追記**: `UI_CONVENTIONS.md`実装手順に「部品スタイルとスキンの上書きが矛盾しそうな場合は実装前にHuman確認」(今回の丸/四角の作り直し防止)。④**コミットノイズ低減**: `scripts/status-clean.ps1`(ジャンクション由来差分を除外して`git status`表示)を追加、VERIFYのGit検査プリセットへ明記。検証: `npm run build`成功、to-top回帰4件成功、temp-smoke成功。

> - **[opencode]** 更新関連バッジを赤色に統一（Human要求、視認性優先）。**対象**: 設定アイコン右上の「更新利用可」点(`FilterBar.tsx`)と設定の更新タブの点(`SettingsOverlay.tsx`)。従来はスキンのアクセント色(`accent-bg`)だったため、テーマによらず見分けにくかった。**変更**: 両方を`bg-red-500 dark:bg-red-400`(アプリ共通の意味色・赤)へ統一。`UI_CONVENTIONS.md`の意味色節と決定済みの判断へ「更新利用可バッジ＝赤で統一」を記録。タイムラインの「記事を更新」ボタン横の点は更新動作中パルスであって通知バッジではないため対象外。検証: `npm run build`成功。バッジ表示は更新が実際に利用可能な状態でのみ描画されるため、E2Eでの視覚確認は未実施（色クラスは既存利用と同一で確定）。

> - **[opencode]** スクロールTOPボタンの位置ズレを修正（Human報告）。**原因**: `.skin-cardinality > :not(.app-content)`／`.skin-ordinary > :not(.ordinary-hud):not(.app-content)`／`.skin-terminal > :not(.terminal-data-stream)` の3ルールがルート直下の全子要素へ`position:relative; z-index:1`を強制し、`position:fixed`の「一番上に戻る」ボタンまで`fixed`→`relative`に上書き(→左下へ移動)。実機E2E(`zz-temp-scroll-top-pos`暫定スペック→使用後に削除)で4スキン(mono/link-amber/ordinary/terminal)のボタンrectを実測し、浮遊スキン(cardinality/ordinary)とterminalで必ず再現、monoは正常と切り分けて特定。**同様のバグ**: ①同じくルート直下で自前の配置(absolute/fixed)を持つ更新通知ポップアップとResizeCornerGuidesも同ルールで壊れていたため併せて修正。②ボタン形状: スタイルスキンの`unlayered :where(button,...)`リセット(`border-radius:0/2px`)が`rounded-full`を打ち負かし、cardinalityで正方形・terminalで角丸2pxに（ordinary/monoは円のまま、と形状がスキン間で不整合）。**修正**: 3スキンの`> :not(...)`に`scroll-to-top-btn`/`update-notice`/`resize-corner-guides`を追い出しで追加、ScrollToTopButton/UpdateNoticePopupへ識別用クラスを付与。ボタン形状は一時`border-radius:9999px!important`で全テーマ円形化したが、Human判断により**テーマ準拠**へ変更（同ルール削除→カーディナリティは四角/ターミナルは角丸2px/モノクロ・オーディナリーは丸=rounded-fullのまま）。検証: `npm run build`成功、4スキンともボタンが`position:fixed`/右下(右マージン16px)/z-40へ戻ることを実測。`scroll-keys.spec.js`(2件)通過、`ui-audit` CLEAR_FAIL=0 AMBIGUOUS=0。FeedPicker/FontPicker(ポータル)・ReaderOverlay(一時text area)はルール対象外と確認。

> - **[opencode]** ワークフロー全体のルーティングを調査し、重複2点を解消（Human承諾）。①**AGENTS.mdとREFACTORING.mdのトリガー二重管理**: AGENTS.mdのRefactoring Triggerを「完全な一覧はREFACTORING.mdのTriggersが正本」という要約に（短縮リストは維持、正本をREFACTORING.mdに一本化）。②**LARGE_CHANGE.mdをIMPLEMENTATION.mdと差分化**: 共通工程（構造確認・検証計画・実装・検証）を複製していたのを「IMPLEMENTATION.mdを参照」にし、大きな変更で追加する観点（Impact & Rollback／Split／広域Verify）だけを残す文書へ書き換え。③**`context/.shared-workflows`の迷子ファイルを調査**: 「`context\workflows`経由で`_shared\workflows`を使う」と書かれた説明メモのみで、ジャンクション本体（`context/workflows`→`_shared/workflows`）は独立して機能し、コード・スクリプト・`_shared`側（finalize-project-move.ps1等）から一切参照されていない。git未追跡・削除しても安全と確認（削除はHuman確認待ち）。ルーティング自体は6workflowが明確なトリガーに対応し適切。

> - **[opencode]** 検証最適化の教訓を文書化（Human承認、参照頻度の観点で配置を選定）。①**罠と対策（プロジェクトのVERIFY.md）**: Tauri/WebView2+WebDriverハーネスでは`$()`/`element.click()`が桁違いに遅い（実測15秒/23秒、スキン非依存）ため、要素の探索・クリックは`browser.execute`+DOM、実入力は`keys`で行う、とE2Eプリセットに追記（実ユーザー操作が要る場合のみ`element.click()`）。②**汎用原則（共有BUG_FIX.md）**: 遅さ・動作不良はアプリ起因とは限らず、環境・ハーネス・ツール起因のことがあるため推測でなく実測で切り分ける、を§2に1行追記。共有workflowのDISCOVERY/IMPLEMENTATION/UNKNOWN_DOMAINは既存の共有由来変更があるため今回のコミット対象外、BUG_FIX.mdは変更が今回分のみのためコミット。

> - **[opencode]** テストの長い待ちの原因を再調査し、**フレームワーク（WebDriver要素操作）起因と特定して修正**。`search-focus.spec.js`の待ちは意図的ではなく、`$()`(findElement)約15.5秒・`element.click()`約23秒がtauri-driver+msedgedriver+WebView2環境で異常に遅いことが原因。**浮遊スキン起因ではなかった**（モノスキンに切替を外しても同時間を計測。前回の「透過ウィンドウ合成」説は誤りと訂正）。一方`browser.execute`/`keys`/DOMクリックは数msと高速。対策: 検索・リーダー系スペックの`$()`/`element.click()`を`execute`+DOMクリックへ置換し、実入力は`keys()`を維持（クリップボードの`navigator.clipboard`もDOMクリックで動作することをreader-readabilityで確認）。結果: `search-focus` 1m36s→2.6s、`regression-check` 2m19s→7.5s、`reader-readability` 44s→5.5s、**フルスイート 6m14s→1m46s**（全9スペック成功、ui-audit AMBIGUOUS=0）。修正ファイル: search-focus/regression-check/reader-readabilityの各spec（高速ヘルパー`waitForSelector`/`clickBySelector`/`readValue`を追加）。

> - **[opencode]** ③浮遊スキンの軽量化を実施（Human承諾）。**A** スポットライトを`requestAnimationFrame`で間引き: 従来はmousemoveごとに`getBoundingClientRect`＋CSS変数2つ書換→全画面radial-gradient再描画。毎フレーム1回にまとめ、レイアウト読取もrAF内へ（`App.tsx`、unmount時cancel）。**B** ぼかし半径を削減: `--plate-blur` 14→9px、オーバーレイ`blur(22px)→14px`。**C** スポットライト半径を200→130px（焦点が明確に）。検証: `npm run build`OK、`regression-check.spec.js`（浮遊スキン含む）通過、`ui-audit` CLEAR_FAIL=0 AMBIGUOUS=0（スポットライト変更はベースラインへ影響なし）。**ただし`search-focus.spec.js`の浮遊スキンE2E所要時間（約1m36s）は不変**で、その主因はWebView2のピクセル単位透過ウィンドウ合成（WS_EX_LAYEREDを外しDWMへ直接合成）というプラットフォーム由来であり、CSS/イベントの軽量化では解消しない（「許容」の判断は維持）。A〜Cは実ユーザーのマウス移動・描画負荷の削減とスポットライトの見た目改善としては有効。

> - **[opencode]** 過去バグの振り返りに基づく対応（Human指摘）。①**検索ロジックの単体テスト化**: `DiscoverOverlay`内にあった`hostOf`/`relevanceOf`を`src/lib/discoverRanking.ts`へ純関数化し、`tests/discoverRanking.test.ts`（`node:test`、`npm run test:unit`で実行、フルE2Eとは独立）を新設。v0.2.10の検索ブレ修正（関連度・二重フィルタ）に決定的回帰を確保。7件成功・`npm run build`OK。②**構造予防パターンを共有workflowへ**: 過去バグの共通原因（非同期レースの応答ガード／二重ソース・識別子の統一／項目は「位置」でなく「ID」で扱う）を共有`workflows/IMPLEMENTATION.md`§7へ追記（汎用教訓のみ、プロジェクト固有の実例はWORKLOG/ARCHITECTURE側）。③**浮遊スキンのui-audit構造監査拡充を調査**: `auditPage()`（DOM監査）はスキン非依存で再利用可、`switchToSkin`もregression-checkに既存。ただし浮遊スキンは描画が重くWebDriverが遅い（search-focus 1m36s）ため、全画面×ピクセル基準は負荷大。軽量案（浮遊スキンのホーム画面をDOM監査のみ・基準なし）が現実的と結論（実装は保留）。④**DB競合仮説を調査**: `paths.rs`の既定ロジックでは開発版(`%APPDATA%\com.noxrss.gyroscope`)とポータブル版(`exe_dir/data`)は**別ディレクトリ**。同一DBを二プロセスで書くのはカスタムdata-dir overrideや非portable版の複数起動等の特定条件のみ。DB接続はWAL未使用・busy_timeout未設定。破損はシャットダウン途中の書き込み・更新再起動・カスタムdir等が候補で、実設定（ポータブルの場所・カスタムdirの有無）のHuman確認が必要。
...
> **概要:** 「一番上に戻る」ボタンの位置ズレを修正。カーディナリティ / オーディナリー / ターミナルのテーマでは、スクロール中にボタンが右下ではなく左下へ移動していた。常に右下に固定されるよう修正。
> 前回リリース(v0.2.10)で実在（ボタンはv0.2.10で追加・その時から）。同原因で更新通知・窓の角つまみも崩れていたため併せて修正。

> **概要:** フローティングテーマ（カーディナリティ / オーディナリー）の軽量化。マウス追従のスポットライトを描画フレーム単位に間引き、ガラスのぼかしを軽くし、スポットライトの焦点を絞った。マウス操作の負荷を抑え、見た目が引き締まった。

> **概要:** 更新のお知らせバッジを赤色に統一。設定アイコン右上と「アップデート」タブの点が赤になり、更新があることを見つけやすくした（従来はテーマ依存の色で見分けにくかった）。
