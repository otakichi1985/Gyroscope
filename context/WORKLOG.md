...
> - **[opencode]** v0.2.11をリリース公開（2026-08-22）。前回v0.2.10からの差分で、エンドユーザー向け変更3件（「一番上に戻る」ボタンの位置ズレ修正＝v0.2.10実在を確認済み／フローティングテーマの動作・見た目改善／更新バッジの赤統一）をリリースノートへ。`git log -S`等で該当バグ(ボタン位置)がv0.2.10に実在することを確認後、Human承認済みノート（内部検証の裏取りは載せない新ルールをVERIFYに追記）で`gh release create v0.2.11`（zip+単体exe添付、HEAD=6f3615fにタグ）。`npm run package:portable`で配布物生成、タグ位置は`git ls-remote`でHEADと一致を確認。URL: https://github.com/otakichi1985/Gyroscope/releases/tag/v0.2.11
