...
<!-- ここから実際の記録 -->
- **[Human]** ジャンル「ゲーム」タブに漫画記事が混ざるのを不審に思い報告。
- **[Claude Code / Sonnet]** 実機確認の結果、Hatenaの`game`スラッグは公式タイトルが「アニメとゲーム」であることが判明（バグではなく表記の誤り）。カテゴリラベルをHatenaの公式タイトルに合わせて修正（`economics`も「経済」→「政治と経済」）。
- **[Claude Code / Sonnet]** 検索結果が少ない件を段階別に計測：独自ポリシーの足切りはほぼ0件、主因はフィード非実在サイトの除外とMAX_CANDIDATES上限。上限を15→30へ引き上げ（実測、速度への影響は軽微）。
