...
**再発防止:**  
`Unreleased Changes`の各エントリを転記する際、必ず「エンドユーザーに見える変更」か「開発工程のみの変更」かを判定し、前者だけを転記する。判定に迷ったら「この挙動をユーザーは目にするか」で分け、目にしないものは除外する。完成後、上記「対象外」の項目が含まれていないことを確認してから公開する。
