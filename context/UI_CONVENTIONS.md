...
5. **部品自身の見た目とスキンの上書きが矛盾しそうな場合は、実装前にHumanへ確認する。**
   例: 部品が`rounded-full`を指定しているのに、スキンの`:where(button)`リセットが角を消す、
   部品が`position:fixed`なのにスキンの`> :not(...)`が`position:relative`を強制する、
   など。どちらの見た目が正解かはスキンの意図に依存するため、AIが一方を決め打ちせず
   先にHumanへ差し示す(類似: IMPLEMENTATION.mdの「操作感の解釈は事前確認」)。
