...
- スキンのアクセント色（所感「味気ない」「各スキンにコントラストを意識した配色を」への対応）:
  ...
  - `Skin.category`で`basic | contrast | style`に分類し、設定画面では3グループに分けて表示する。
    `visualStyle`を持つオーロラ／ベルベット／ターミナル／リンク・アンバーだけが配色以外の
  ...
  - リンク・アンバーは白い浮遊パネル、薄灰の構造線、山吹色の選択表示を持つスタイルテーマ。
    作品固有のロゴやアイコンは複製せず、テーマ選択時の短いトレース線とカードhover時の
    横移動だけでリンク操作の軽快さを表す。白いパネル表現を守るため選択中だけライト固定だが、
    保存済み`themeMode`は変更せず、別テーマへ戻したときに復元する
