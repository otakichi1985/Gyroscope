...
- スキンスウォッチの配色修正（実機報告「トーンが高すぎて薄い色すぎて見づらい」）:
  `SettingsOverlay.tsx`のスキン選択ドットは元々`skin.light`（パネル背景用の、あえて
  白に近い薄い色）をそのまま表示していたため、4種類とも似たような薄い円にしか見えなかった。
  パネル背景色ではなくアクセント色（`skin.accentLight`/`accentDark`、選択中タブなどに
  使っている彩度の高い方の色）を表示するよう変更。ただし`.accent-*`ユーティリティは
...
- スキンの配色は**目視ではなく実測（WCAGコントラスト比）で決めること**（`lib/skins.ts`）:
  - アクセント色は2つの役割を同時に負っている。**片方を忘れやすいので必ず両方測る**。
    ...
    2. `accent-bg`のベタ塗りボタンに載る**白文字**（`StatePanel`のCTA、`ReaderOverlay`の
       「ブラウザで全文を読む」、トグルのつまみ）。こちらも**4.5:1**
