...
- スキンのアクセント色（所感「味気ない」「各スキンにコントラストを意識した配色を」への対応）:
  ...
  - カーディナリティは白銀の浮遊パネルと鮮やかな金色の接続表示を持つVR UIテーマ。
    可読文字用の濃い`accentLight`と、演出用の`--cardinal-gold`を分離して金色が茶色く沈まないようにする。
    テーマ選択時の接続線とカードhover時のリンク表示を持つ。白いパネル表現を守るため選択中だけ
    ライト固定だが、保存済み`themeMode`は変更せず、別テーマへ戻したときに復元する
  - オーディナリーは角を切った青白いホログラム面、同心円、走査光を持つAR UIテーマ。
    選択中はダーク固定。同心円の回転を除く演出は接続時／hover時だけにし、
