...
## 進行中の検討（BOOTHショップ監視機能、未実装）

友人から「特定のBOOTHショップの更新を検知して、リンクを踏めるようにしたい」という要望があり、
実装するかどうかを検討中。次のセッションで再開する際はここから読めば経緯が追える。

- **前提として確認済みの事実**:
  - BOOTHのショップページはRSS/Atomフィードを一切提供していない（`<link rel="alternate">`
    無し、`/rss`・`/feed`等の一般的なパスも全部404）
  - BOOTH自体の`robots.txt`はショップ・商品ページのクロールを禁止していない
    （`/terms`・`/cart`系のみ除外）。2025年10月の公式告知でも「常識的な範囲の
    スクレイピング」を許容する方向にガイドラインを見直すと明言されている
  - **ただし手前にCloudflareのbot対策が立っており、`reqwest`のような素のHTTP
    クライアントでは`403`＋「Just a moment...」のJS challengeページで止まる**
    （`curl`で実機確認済み、レスポンスヘッダーに`Cf-Mitigated: challenge`が付く）。
    これはBOOTHの意思とは別レイヤーの話で、robots.txtが許可していても関係なく弾かれる
  - **このJS challengeは、Tauriアプリ自身が元々持っているWebView2（本物のChromiumエンジン）
    で開けば通過できることを実機で確認済み**（`src-tauri/src/commands/booth_test.rs`の
    使い捨てテストコマンドで検証。裏で`WebviewWindowBuilder`から2つ目のウィンドウを開いて
    ショップページを読み込ませたところ、商品一覧が正常に表示された）。つまり素のHTTPで
    取得する設計は不採用、**WebView2越しに開いてDOMから読み取る**方式が前提になる
- **要望を分解した4項目**（この順で難易度が下がる）:
  1. Cloudflareの通過 → 解決済み（上記の通り）
  2. 特定ショップURLの登録＋定期監視＋新着判定 → 未着手。前回の議論（Gemini/Opus5/ChatGPT
     に相談した内容）は下記参照
  3. 検出した商品のリンクをクリックで開けるようにする → ほぼゼロコスト。既存の
     「記事を開く方法（ブラウザ/アプリ内リーダー）」の仕組みをそのまま使える
  4. 商品画像も表示 → 独立した課題ではなく、2の抽出処理に画像URLも含めるだけ
- **他のAI（Gemini/Opus5/ChatGPT）に相談して出てきた設計方針**（Cloudflareの件が
  判明する前の相談だが、抽出まわりの設計自体はそのまま有効）:
  - 「BOOTH専用コードをベタ書き」ではなく、RSSと共通の内部データ形（記事ID・タイトル・
    URL・サムネ等を持つ正規化された型）に変換してから、その先（DB保存・既読・通知・
    表示）は今のRSSパイプラインをそのまま使い回す設計にする。ChatGPT案は`FeedSource`
    トレイト＋`SourceItem`型、Opus5/Gemini案は共通の抽出エンジン＋サイトごとの
    「レシピ」（TOML等の設定ファイルでCSSセレクタ相当を指定）という、狙いは同じで
    実装の重心が違う2つの案
  - レシピ（設定ファイル）方式の方が、BOOTH側のマークアップが変わって直す時に
    **アプリを再ビルド・再配布せずレシピ更新だけで直せる**という利点があり、こちらの
    方が筋が良さそうという結論に傾いていた（ユーザーとの合意）
  - 実装上の注意点（3人とも指摘）: 商品IDをGUIDにする（タイトル・価格は変わりうる）／
    初回購読時は既存商品を全部既読扱いにしてから差分監視を始める／「0件パースできた」を
    「全部消えた」ではなく「パースが壊れた」として扱う（サイレント失敗が一番まずい）／
    ポーリング間隔は控えめに（数十分〜）＋ETag等でのキャッシュ利用／新商品と値下げ・
    再入荷を別種のイベントとして扱い、最初は新商品だけ通知する
  - 選択肢として出ていた「RSSHubに`booth.pm/shop/:subdomain`という既存ルートがあるかも
    しれない」という近道は、Cloudflareの件が判明する前の情報でまだ検証していない。
    次に着手する際、自前実装の前に一度試す価値はある（動けば実装コストゼロ）
- **未着手・次にやること**: WebView2越しの取得を実際の定期監視・データ抽出・DB保存
  にまで発展させる設計と実装。`booth_test.rs`と`SettingsOverlay.tsx`の
  `BoothScrapeTestSection`は検証専用の使い捨てコードなので、本実装に進む場合は
  作り直すか、たたき台として発展させるか判断すること（現状は消さずに残している）
