...
このファイルは移行前の `CLAUDE.md` を内容保持のため退避した参考資料であり、正本・現在の指示ではない。
現在の事実と開発ルールは `../AGENTS.md` および同階層の正本文書、コード、Gitを優先する。
...
- アプリ内蔵の自動更新チェック（配布先をGoogle Driveから非公開のGitHubリポジトリ
  ...
  - 適用は「新しいexeをダウンロード→今動いているexeを`.exe.bak`にリネーム→ダウンロード
    ...
    **DB（`app_data_dir()`配下）やlocalStorage・設定は一切触らない**（実行ファイルの
    リネーム/差し替えはexeと同じフォルダ内で完結する）ため、アップデートしても
    フィード購読・既読状態・外観設定などは保持される
  - **`apply_update`/`rollback_update`/`restart_app`はいずれも`app.exit(0)`直前で
    `Db`のMutexを取得してから抜ける。** `app.exit()`はDropを経由せず即座にプロセスを
    終了させるため、バックグラウンドスケジューラ（60秒ごとのフィード更新）がちょうど
    同じ接続に書き込み中のタイミングと衝突しうる ―― `npm run tauri dev`のファイル監視が
    プロセスを強制終了して`database disk image is malformed`を引き起こした既知の落とし穴と
    同じ系統のリスクなので、同じ轍を踏まないよう明示的にロックを取ってから終了する
    （書き込み中のスレッドがあればそこで一瞬待ってから終了する形になる）
...
友人からの「特定のBOOTHショップの更新を検知して、リンクを踏めるようにしたい」という要望を
実装済み。前提（BOOTHはRSSを提供しない、手前にCloudflareのJS challengeがあり素の`reqwest`
では`403`で止まる、WebView2越しなら通過できる）は前セッションの検証済み。以下は実際の
定期監視・抽出・保存・表示まで組んだ際の設計と、実機で踏んだ罠。

- **抽象化を増やさない設計**: `NewEntry`（`db/models.rs`）はguid/title/link/summary/
  thumbnail_url等しか持たず元からRSS固有の情報が無いため、専用トレイトや複数ソース
  エンジンを新設せず、`commands::feeds::refresh_feed_inner`（fetch→parse→
  `upsert_entries`→通知を一元管理する既存の1関数）の中で`feeds.source_type`
  （`rss` | `booth`、`db/migrations.rs` v5で追加）によって「取得方法」だけを分岐させ、
  その後のDB保存・既読・通知・表示は完全に共通のまま。分岐の中身は
  `apply_rss_outcome`/`apply_booth_outcome`という同じ形の2関数に切り出し、
  どちらも`(Vec<NewEntry>, feed_content_changedかどうか)`を返す
- **取得: `src-tauri/src/fetch/booth.rs`**。`WebviewWindowBuilder`でショップURLを開き、
  Cloudflareの通過を待ってDOMから商品一覧を抽出する。IPC（`invoke`）は使っていない
  ―― 外部オリジン（`booth.pm`）にIPCを許可するには`dangerousRemoteDomainIpcAccess`が
  要り、単一サイトの監視機能のために「BOOTH側のJSがこのアプリのコマンドを呼べる」経路を
  開くのは見合わないため。代わりに、注入したJSが`history.replaceState`で結果をURL
  フラグメントに書き込み（`document.title`だと長さが心もとないため）、Rust側は
  `window.url().fragment()`を0.75秒間隔でポーリングする。Cloudflareの本物のページへの
  リダイレクトはドキュメントを丸ごと入れ替えるため、抽出スクリプトは**毎ティック
  打ち直す**（1回注入して`setTimeout`で待たせる方式だとリダイレクトでタイマーごと
  消える）
  - ウィンドウは`.visible(true)`のまま`.position(-32000.0, -32000.0)`で仮想デスクトップ外へ
    出し`.skip_taskbar(true)`を付ける形でユーザーから隠す。`.visible(false)`にしない理由は、
    Cloudflare突破が確認済みなのは可視ウィンドウの構成だけで、不可視だと
    `document.visibilityState`のようなJSから見える差分をCloudflareが見ている可能性が
    あるため（未検証のリスクを増やさず、実証済みの構成を保ったまま位置だけ変える）
  - 同時実行数は`BoothScrapeLimiter`（`Semaphore(1)`、`app.manage`）で1に絞っている。
    `scheduler::MAX_CONCURRENT_FETCHES`（RSS用、6）とは別軸 ―― WebView2は1枚開くだけでも
    reqwestの1リクエストよりずっと重く、複数枚同時にCloudflareへ突撃すると弾かれやすくなる
  - 抽出セレクタは`Recipe`構造体（既定値をRustに埋め込み）で、データディレクトリの
    `booth_recipe.json`があればフィールド単位で上書きできる。BOOTHのマークアップが
    変わったときにアプリの再ビルド無しで直せるようにするための最小限の仕組み
    （複数サイト対応の汎用エンジンは作っていない、今回のスコープはBOOTHのみ）
  - 「0件パース」と「壊れている」を区別する: 抽出結果に商品が0件でも、ショップの
    ルートコンテナ（`#js-shop`等）が見つかっていれば正当な「商品0件のショップ」として
    `Ok(vec![])`、ルートも見つからなければ`BoothScrapeError::StructureMismatch`として
    `feeds.last_error`に出す（RSSのパース失敗と同じ扱い、サイレント失敗にしない）
  - **【実機で踏んだ罠】抽出スクリプトが読み込み完了前の1回目のティックで即決していた。**
    当初は「`document.title`が"Just a moment"を含む間だけ待つ」という判定だけだったが、
    実機では"Just a moment"を経由しない（あるいはタイトルが変わる前に）ページがまだ
    読み込み中の状態で抽出スクリプトが1回目のティックで即座に「0件・ルートも無し」と
    確定し、`StructureMismatch`を返してしまった（実際のDOMをブラウザで確認したところ
    既定セレクタ自体は正しくBOOTHの実マークアップ（`li.js-mount-point-shop-item-card`等）
    にヒットする構造だったため、セレクタの誤りではなく判定タイミングの問題と判明）。
    `window.__boothTick`（同一ドキュメントに対する`.eval()`の再実行のたびに増分、
    本物のCloudflareリダイレクトでドキュメントが入れ替われば自然にリセットされる）を
    導入し、0件・ルート無しの場合は`MIN_TICKS_BEFORE_GIVE_UP`（16ティック≒12秒、
    実測されたCloudflareの待ち時間8秒に余裕を持たせた値）に達するまで確定を待つように
    変更。テンプレートに`__MIN_TICKS__`のようなプレースホルダを足したのに
    `build_extraction_script`の`.replace()`チェーンへ足し忘れる、というミス自体も
    実際にやってしまった（Rustのコンパイルは通ってしまい、実行時にJSの
    `ReferenceError`が握りつぶされて`InvalidPayload`に化けるだけなので気づきにくい）。
    再発防止に`extraction_script_has_no_unreplaced_placeholders`というテストを追加した
    （プレースホルダの一覧を静的に持ち、生成後のスクリプト文字列に残っていないか確認する）
- **`add_feed`でのBOOTH判定**: 新しい入力欄は作らず、既存の1つのURL欄にBOOTHショップURLを
  貼っても動くようにした。`is_booth_shop_host`がホスト名が`*.booth.pm`のサブドメイン
  （`www`/`s`/`assets`/`manage`/`accounts`等の既知の非ショップサブドメインを除く）かで
  判定し、該当すれば`add_booth_feed`（スクレイプ→`upsert_entries`→初回分を既読化→
  `interval_min`に既定30分ではなく60分を設定）に分岐する。**`notify_enabled`もBOOTHだけ
  既定でオン**にしている（他の全フィードは既定オフ）。BOOTHショップ監視は「新着に気付きたい」
  こと自体が目的なので、他フィードと同じ「まず追加→気付いたら通知をオンにする」の
  2段階にすると本末転倒になるため、ここだけ意図的に既定を変えている（フィードごとに
  後から切り替えは可能、既存の通知トグルUIをそのまま使う）
  - **初回購読時は既存商品をすべて既読扱いにする**が、`commands::entries::mark_all_read`
    は流用していない ―― あの関数は`read_history`にもスナップショットを書くため、
    ショップの全カタログを「読んだ履歴」として残してしまう（実際には読んでいない）。
    代わりに`UPDATE entries SET is_read = 1 WHERE feed_id = ?1`を直接叩くだけにして、
    `read_history`を汚さないようにしている（実機で`read_history`に増分が無いことを確認済み）
  - ショップアイコンは`favicon::discover_favicon`（reqwestベース、Cloudflareに阻まれる）を
    使わず、スクレイプ自体でページの`og:image`/`<link rel=icon>`相当を拾って
    `icon_path`に`COALESCE`で書き込む。取得できなければ次回以降の成功したスクレイプで
    再挑戦され続ける（一度きりのバックフィルより単純で、失敗を引きずらない）
  - `backfill_favicons`（起動時の一括favicon探索）は`source_type != 'booth'`を条件に
    追加し、BOOTHフィードをそもそも対象から外している（どのみち失敗するだけの
    reqwestリクエストを毎起動投げない）
- **フロントエンド**: `Feed`型に`source_type`を追加した以外はほぼ無改造。サムネイル表示・
  クリックで開く・スター・既読は元から`entry.thumbnail_url`/`entry.link`しか見ない
  汎用実装だったため、BOOTH商品も新しいコードパス無しでそのまま表示・クリック可能になった
  （「リンクを踏める」「画像も表示」という要望はここに手を入れずに満たされた）。
  `FeedManager.tsx`のフィード一覧にはBOOTHフィードだけ小さな"BOOTH"バッジを付けている
  （`interval_min`が60分固定で始まる理由がひと目でわかるように）
- **使い捨てプロトタイプの削除**: `src-tauri/src/commands/booth_test.rs`と
  `SettingsOverlay.tsx`の`BoothScrapeTestSection`/`boothTest`セクションは、
  結論（Cloudflareは突破できる）が出たので削除した
- **未検証のまま残していること**: 「RSSHubに`booth.pm/shop/:subdomain`という既存ルートが
  あるかもしれない」という近道は今回も試していない（動けば実装コストゼロだが、
  スクレイプ経路が既に動いているため優先度を上げなかった）。値下げ・再入荷の検知は
  スコープ外のまま（新商品の検知のみ）
